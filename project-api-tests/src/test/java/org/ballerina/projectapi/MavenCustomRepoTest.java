...
import static org.ballerina.projectapi.MavenCustomRepoTestUtils.*;
...
public class MavenCustomRepoTest {
    ...
    private static final String packagename = "pkg3";
    ...
    public void setUp() throws IOException, InterruptedException {
        ...
        try {
            ...
            System.err.println("Warning: failed to delete existing GitHub package artifacts: " + e.getMessage());
        }
    ...
    public void testPushBalaGithub() throws IOException, InterruptedException {
        ...
        String expectedBalaPath = "target/bala/" + org + "-" + packagename + "-" + platform + "-" + version + ".bala";

        Assert.assertTrue(collapsed.contains("Creating bala") && collapsed.contains(expectedBalaPath),"Expected creation message with path: " + expectedBalaPath + "\nActual output: " + buildOutput);
        args = new ArrayList<>();
        ...
        Assert.assertTrue(collapsedPush.contains("Successfully pushed target/bala/" + org + "-"
    ...
    public void testBuildBalaGithubOnline() throws IOException, InterruptedException {
     ...
     public void publishBalaPackagesBeforeTests() throws IOException, InterruptedException {
        ...
        String[][] seq = {
         ...
         for (String[] entry : seq) {
             String pkg = entry[0];
             String ver = entry[1];
             Process pack = packTrigger(pkg, this.tempWorkspaceDirectory, this.envVariables);
    ...
    public void testCase1_buildMyProject_assertDependenciesToml() throws IOException, InterruptedException {
         ...
         Optional<String> pkg1Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg1");
         Optional<String> pkg2Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg2");

         Assert.assertTrue(pkg1Version.isPresent(), "Expected pkg1 to be present in Dependencies.toml");
         Assert.assertEquals(pkg1Version.get(), "0.1.0", "Package version is not matching with the pushed package version");
         Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
         Assert.assertEquals(pkg2Version.get(), "1.0.0", "Package version is not matching with the pushed package version");
    }

    @BeforeGroups("testCase2")
    public void beforeGroupTestCase2() throws IOException, InterruptedException {
   ...
   @Test(description = "Push more packages to Github to test locking mode results" , dependsOnMethods = "testCase1_buildMyProject_assertDependenciesToml")
   public void testCase2_publishAdditionalVersionsForDeps() throws IOException, InterruptedException {
       ...
       for (String ver : pkg1Versions) {
           ...
           Process p = packTrigger("pkg1", this.tempWorkspaceDirectory, this.envVariables);
           p = pushTrigger("pkg1", this.tempWorkspaceDirectory, this.envVariables);
           String pushOutput = getString(p.getInputStream());
           // Normalize output so Windows backslashes don't break the assertion
           String normalizedPushOutput = pushOutput.replace("\\", "/").replace("\r\n", "\n").replace("\r", "\n");
           String collapsedPush = normalizedPushOutput.replaceAll("\\s+", " ");
           Assert.assertTrue(collapsedPush.contains("Successfully pushed target/bala/" + org + "-pkg1-any-" + ver + ".bala to '" + GITHUB_REPO_ID + "' repository."),
                   "Expected push success message. Actual output: " + pushOutput);
       }
       ...
       for (String ver : pkg2Versions) {
           ...
           String normalizedPushOutput = pushOutput.replace("\\", "/").replace("\r\n", "\n").replace("\r", "\n");
           String collapsedPush = normalizedPushOutput.replaceAll("\\s+", " ");
           Assert.assertTrue(collapsedPush.contains("Successfully pushed target/bala/" + org + "-pkg2-any-" + ver + ".bala to '" + GITHUB_REPO_ID + "' repository."),
                   "Expected push success message. Actual output: " + pushOutput);
       }
   ...
   @Test(description = "Build package with soft locking mode" , dependsOnMethods = "testCase2_publishAdditionalVersionsForDeps", groups = "testCase2")
    ...
    List<String> args = new ArrayList<>();
       ...
       if(dependencyPathBefore.exists() && dependencyPathBefore.delete()){
           ...
           Process clean  = executeCleanCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"),cleanArgs, this.envVariables);
    ...
    Assert.assertTrue(collapsed.contains("Generating executable") && collapsed.contains("target/bin/myproject1.jar"),
       ...
       Optional<String> pkg2Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg2");
       Assert.assertTrue(pkg1Version.isPresent(), "Expected pkg1 to be present in Dependencies.toml");
       Assert.assertEquals(pkg1Version.get(),"0.1.1","Package version is not matching with the expected package version");
       Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
       Assert.assertEquals(pkg2Version.get(),"1.1.1","Package version is not matching with the expected package version");
   }

   @Test(description = "Build package with medium locking mode" , dependsOnMethods = "testCase2_publishAdditionalVersionsForDeps", groups = "testCase2" )
   public void testCase2_2_mediumLockingMode_resolvesConservative() throws IOException, InterruptedException {
    ...
    Assert.assertEquals(pkg1Version.get(),"0.1.1","Package version is not matching with the expected package version");
    Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
    Assert.assertEquals(pkg2Version.get(),"1.0.0","Package version is not matching with the expected package version");
   }
   ...
   @Test(description = "Build package with hard locking mode" , dependsOnMethods = "testCase2_publishAdditionalVersionsForDeps", groups = "testCase2")
   public void testCase2_3_hardLockingMode_enforcesExact() throws IOException, InterruptedException {
    ...
    Assert.assertTrue(collapsed.contains("Generating executable") && collapsed.contains("target/bin/myproject1.jar"),
       ...
       Optional<String> pkg2Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg2");
       Assert.assertTrue(pkg1Version.isPresent(), "Expected pkg1 to be present in Dependencies.toml");
       Assert.assertEquals(pkg1Version.get(),"0.1.0","Package version is not matching with the expected package version");
       Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
       Assert.assertEquals(pkg2Version.get(),"1.0.0","Package version is not matching with the expected package version");

    }

   @Test(description = "Build package with locked mode" , dependsOnMethods = "testCase2_publishAdditionalVersionsForDeps", groups = "testCase2")
   ...
   public void testCase3_backwardCompatibility_legacyBallerinaToml() throws IOException, InterruptedException {
        ...
        if (Files.deleteIfExists(dependencyPath)) {
            ...
            List<String> cleanArgs = new ArrayList<>();
            Process clean = executeCleanCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"),
                    cleanArgs, this.envVariables);
            String cleanErrors = getString(clean.getErrorStream());
            if (!cleanErrors.isEmpty()) {
                Assert.fail(OUTPUT_CONTAIN_ERRORS + cleanErrors);
            }
            ...
            Process build = executeBuildCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"),
             ...
             Assert.assertTrue(collapsed.contains("Generating executable") && collapsed.contains("target/bin/myproject1.jar"),
                     "Expected generating executable + target/bin/myproject1.jar in output. Actual output: " + buildOutput);
             Path dependencyPath2 = this.tempWorkspaceDirectory.resolve("myproject1").resolve("Dependencies.toml");
             Optional<String> pkg2Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath2, "pkg2");
             Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
             Assert.assertEquals(pkg2Version.get(),"1.1.1","Package version is not matching with the expected package version");
        }
        else{
    ...
    public void pinPkg1AndPkg2To100() throws IOException, InterruptedException {
        Path dependencyPath = this.tempWorkspaceDirectory.resolve("myproject1").resolve("Dependencies.toml");
        if(Files.exists(dependencyPath) && Files.deleteIfExists(dependencyPath)){
            List<String> cleanArgs = new ArrayList<>();
            Process clean  = executeCleanCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"),cleanArgs, this.envVariables);
            String cleanErrors = getString(clean.getErrorStream());
            if (!cleanErrors.isEmpty()) {
        ...
        List<String> args = new ArrayList<>();
        args.add("--locking-mode=hard");
        Process build = executeBuildCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"), args, this.envVariables);
        String buildErrors = getString(build.getErrorStream());
        if (!buildErrors.isEmpty()) {
            Assert.fail(OUTPUT_CONTAIN_ERRORS + buildErrors);
        }
   ...
   // New: run this setup only before the Test Case 4 group
    ...
    @Test(description = "Test case 4.1: Add dep3 and build with SOFT locking mode", dependsOnGroups = "testCase2", groups = "testCase4")
     public void testCase4_1_addDep3_softLocking() throws IOException, InterruptedException {
        ...
        Optional<String> pkg3Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg3");
        Optional<String> pkg2Version = MavenCustomRepoTestUtils.getPackageVersionFromDependencies(dependencyPath, "pkg2");
        Assert.assertTrue(pkg2Version.isPresent(), "Expected pkg2 to be present in Dependencies.toml");
        Assert.assertEquals(pkg2Version.get(),"1.1.1", "Pkg3 version should be 1.1.1");
    ...
    @Test(description = "Test case 4.2: Add dep3 and build with MEDIUM (default) locking mode", dependsOnGroups = "testCase2", groups = "testCase4")
    ...
    @Test(description = "Test case 4.3: Add dep3 and build with HARD locking mode", dependsOnGroups = "testCase2", enabled = true, groups = "testCase4")
    ...
    @Test(description = "Test case 4.4: Add dep3 and build with LOCKED locking mode", enabled = true,
     ...
     public void testCase4_4_addDep3_lockedLocking() throws IOException, InterruptedException {
        ...
        boolean stderrHasLockedMsg = buildErrors != null && (buildErrors.contains("cannot add new imports with --locking-mode=locked")
        ...
        Assert.assertFalse(collapsed.contains("Generating executable") && collapsed.contains("target/bin/myproject1.jar"),
                "Build unexpectedly succeeded in locked locking mode when adding new imports. stdout: " + collapsed + "\nstderr: " + buildErrors);
    }

    @Test(description = "Build Package With More Versions", dependsOnMethods = "testCase2_publishAdditionalVersionsForDeps")
    public void testCase5_addVersionInBallerinaToml_buildSucceeds() throws IOException, InterruptedException {
        ...
        updateVersionForPackage(this.tempWorkspaceDirectory.resolve("myproject1"), "pkg2", "1.1.0");
        List<String> args = new ArrayList<>();
        Process build = executeBuildCommand(DISTRIBUTION_FILE_NAME, this.tempWorkspaceDirectory.resolve("myproject1"),
                args, this.envVariables);
        String buildErrors = getString(build.getErrorStream());
    ...
    public void testCase5_addIncompatibleVersion_buildFails() throws IOException, InterruptedException {
        // Ensure Dependencies.toml exists so we can verify an incompatible Ballerina.toml change against a locked state.
        Path dependencyPath = this.tempWorkspaceDirectory.resolve("myproject1").resolve("Dependencies.toml");
        ...
        String existingVersion = getPackageVersionFromDependencies(dependencyPath, "pkg2").orElse("");

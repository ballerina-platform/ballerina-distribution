...
function matchFn1(Pair pair) {
    match pair {
        // The binding pattern below matches mappings that contain at least the fields with `x` and `y` keys.
...
function matchFn3(Rec r) {
    match r {
        // The pattern below matches a mapping that has a field with `p` key and a value that is another

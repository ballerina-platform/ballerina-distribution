...
service websub:SubscriberService /websub on websubEP {
    ...
    // Defines the remote function that accepts the intent verification requests.
    // If the remote function is not specified, intent verification happens automatically. It verifies if the topic

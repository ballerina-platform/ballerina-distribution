...
public function main() {
    ...
    // It can occur multiple times anywhere between from and select clause
    // This will pass the frame to select clause only if `i % 2 == 0` is true
    int[] evenNums = from int i in nums
                     where i % 2 == 0
                     select i;
    io:println(evenNums);
 
    // `order by` clause can be used to sort the result in 
    ...
    // Iterating a string value using query expression

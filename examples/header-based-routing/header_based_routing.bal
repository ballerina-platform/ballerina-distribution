...
service headerBasedRouting on new http:Listener(9090) {
    // [http:ResourceConfig](https://ballerina.io/learn/api-docs/ballerina/http/records/HttpResourceConfig.html) annotation with 'GET' method declares the
    // HTTP method.
    @http:ResourceConfig {
        methods: ["GET"],
        path: "/route"
    ...
    resource function hbrResource(http:Caller caller, http:Request req) {
        ...
        //[getHeader()](https://ballerina.io/learn/api-docs/ballerina/http/objects/Request.html#getHeader) returns header value of the specified header name.
        string nameString = req.getHeader("x-type");

        http:Response|error response;
        if (nameString == "location") {
            //[post()](https://ballerina.io/learn/api-docs/ballerina/http/clients/Client.html#post) remote function represents the 'POST' operation
            // of the HTTP client.
            ...
            //[get()](https://ballerina.io/learn/api-docs/ballerina/http/clients/Client.html#get) remote function can be used to make an http GET call.
            response =
        ...
        if (response is http:Response) {
            // [respond()](https://ballerina.io/learn/api-docs/ballerina/http/clients/Caller.html#respond) sends back the inbound clientResponse to the caller
            // if no error occurs.

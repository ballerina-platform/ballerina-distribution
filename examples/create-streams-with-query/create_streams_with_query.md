...
Query expression can be used to create streams. Query expression should be preceded by the `stream` keyword in this case. If the expected stream type is `stream<T>` then the `select` clause must return values belonging to `T`.

...
The `service` keyword is used with class definition to define service class. A service object can be constructed from service class. A service object supports network interaction with a remote system that is originated by the remote system. They are a special kind of objects that contain `remote` and `resource` methods in addition to regular methods.

The `Listener` object receives network input and makes calls to remote methods of attached service objects. 
The below example demonstrates how to start the service from its class definition.

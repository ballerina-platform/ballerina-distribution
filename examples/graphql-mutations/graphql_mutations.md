...
**Note:** GraphQL mutations are expected to perform side-effects on the underlying data system, i.e. mutations are expected to mutate the state of the backend. In Ballerina, `remote` methods are used to handle such cases. Therefore, `remote` methods are used to represent the fields of GraphQL `Mutation` type.

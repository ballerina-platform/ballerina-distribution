...
The request payload data binding also validates the payload against any constraints defined on the request payload type. If the validation fails, a 400 Bad Request response will be returned to the client with the validation error message. Use this to directly validate the request payload with respect to the defined constraints.

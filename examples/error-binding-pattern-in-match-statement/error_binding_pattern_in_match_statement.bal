...
function errorMatch(error v) {
    match v {
        ...
        // binds its `code` detail mapping to `code` variable.
        var error DatabaseError(code = code) => {
            io:println("Matched DatabaseError with code:", code);
        }
        // This error binding pattern matches an error value with `ERROR` error message and
        // binds its `code` detail mapping to `code` variable.
        var error(ERROR, code = code) => {
            io:println("Matched Generic Error with message:", ERROR, " and code:", code);
        }
        // This error binding pattern matches the value of the error message in an error with `message`
        // variable, the value of the field `type` in the detail mapping is matched with `errorType` variable and
        // the rest parameter `...otherDetails` is matched with the remaining detail fields.
